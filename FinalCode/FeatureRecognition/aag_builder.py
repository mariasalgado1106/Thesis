...
def extract_mesh_data(shape):
    vertices, triangles, vertex_count = [], [], 0
    ...
    for face in topo.faces():
        ...
        for i in range(1, triangulation.NbTriangles() + 1):
            ...
            if face.Orientation() == TopAbs_FORWARD: triangles.append([face_vertices[n1 - 1], face_vertices[n2 - 1], face_vertices[n3 - 1]])
            else: triangles.append([face_vertices[n1 - 1], face_vertices[n3 - 1], face_vertices[n2 - 1]])
...
class AAGBuilder_3D:
    ...
    def visualize_3d_aag(self, show_mesh=True, mesh_opacity=0.2, node_size=10, hide_convex=False):
        ...
        for ftype in sorted(set(face_types)):
            if hide_convex: # keep only non‑stock faces
                indices = [f['index'] for f in self.face_data_list if f['type'] == ftype and f['stock_face'] != "Yes"]
            else: indices = [f['index'] for f in self.face_data_list if f['type'] == ftype]
        ...
        for edge in self.edge_data_list:
            for pair in edge['faces_of_edge']:
                ...
                if idx_pair in drawn_pairs: continue
                drawn_pairs.add(idx_pair)
                classification = 'Unknown'
                if edge['classification']: classification = edge['classification'][0]
                if hide_convex and classification == 'Convex': continue
                group = edge_groups.get(classification, edge_groups['Unknown'])
                group['edges'].append((i, j, edge))
        for group in edge_groups.values():
            if not group['edges']: continue
...
class AAGBuilder_2D:
    ...
    def build_aag_subgraph (self):
        if self.G is None: self.build_aag_graph()
        def filter_stock_faces(node): return self.G.nodes[node].get("stock_face") == "No"
        ...
        def filter_edge_not_all(n1, n2): #only removes convex edges connected to stock faces
            ...
            if edge_data.get("edge_type") != "convex": return True
            ...
            if is_n1_stock or is_n2_stock: return False
            return True
        ...
        for component in nx.connected_components(view):
            if len(component) > 1: nodes_to_keep.extend(component)
            else:
    ...
    def analyse_subgraphs(self):
        if self.subG is None: self.build_aag_subgraph()
    ...
    def visualize_2d_aag (self):
        ...
        for n in self.G.nodes:
            ...
            if face_type == 'Plane': node_colors_G.append(self.colors_rgb['geo_plane'])
            elif face_type == 'Cylinder': node_colors_G.append(self.colors_rgb['geo_cylinder'])
            else: node_colors_G.append(self.colors_rgb['geo_other'])
        node_colors_subG = []
        for n in self.subG.nodes:
            face_type = self.subG.nodes[n].get('face_type', 'Other')
            if face_type == 'Plane': node_colors_subG.append(self.colors_rgb['geo_plane'])
            elif face_type == 'Cylinder': node_colors_subG.append(self.colors_rgb['geo_cylinder'])
            else: node_colors_subG.append(self.colors_rgb['geo_other'])
        # 2.2: Edge colors
        edge_colors_G = []
        for x, y, z in self.G.edges(data=True):
            edge_type = z.get('edge_type', 'other')
            if edge_type == 'convex': edge_colors_G.append(self.colors_rgb['edge_convex'])
            elif edge_type == 'concave': edge_colors_G.append(self.colors_rgb['edge_concave'])
            elif edge_type == 'tangent':  edge_colors_G.append(self.colors_rgb['edge_tangent'])
            else: edge_colors_G.append((0.5, 0.5, 0.5))  # grey for unknown
        edge_colors_subG = []
        for x, y, z in self.subG.edges(data=True):
            edge_type = z.get('edge_type', 'other')
            if edge_type == 'convex': edge_colors_subG.append(self.colors_rgb['edge_convex'])
            elif edge_type == 'concave': edge_colors_subG.append(self.colors_rgb['edge_concave'])
            elif edge_type == 'tangent': edge_colors_subG.append(self.colors_rgb['edge_tangent'])
            else: edge_colors_subG.append((0.5, 0.5, 0.5))  # grey for unknown
