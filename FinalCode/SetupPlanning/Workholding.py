...
class Workholding:
        ...
        for v1 in dim1_range:
            for v2 in dim2_range:
                in_face1 = any(self.setup_plan._is_point_in_face_mesh(v1, v2,self.face_data_list[f]['mesh_vertices'],
                                                        self.face_data_list[f]['mesh_triangles'], idx1, idx2) for f in faces1)
                if in_face1:
                    in_face2 = any(self.setup_plan._is_point_in_face_mesh(v1, v2,self.face_data_list[f]['mesh_vertices'],
                                                        self.face_data_list[f]['mesh_triangles'], idx1, idx2) for f in faces2)
                    if in_face2:
                        res_pnt = [0.0, 0.0, 0.0]
                        res_pnt[idx1] = v1
                        res_pnt[idx2] = v2
    ...
    def clamping_faces (self):
        vice_library = {'width': 200,'height': 50,'length': 150}
        ...
        for setup in self.optimized_plan:
            ...
            for fa1,fa2 in pairs_parallel_faces: #fa = face axis
                ...
                # 5. Area -> clamp only up to h_filt
                ...
                # Print Row
                pair_str = f"{fa1}/{fa2}"
                print(f"{setup_axis:<8} | {pair_str:<10} | {clamping_width:<8.2f} | {h_max:<8.2f} | {max_len:<8.2f} | {h_ratio:<8.2f} | {len_ratio:<8.2f} | {hanging_height_length_ratio:<8.2f} | {bbox_area_ratio:<8.2f} | {status}")
    ...
    def visualize_common_area(self, axis1, axis2, common_points):
        ...
        def add_trace(pts, name, color, opac, size):
            if pts is None or not isinstance(pts, (list, np.ndarray)): return
            if len(pts) == 0: return
